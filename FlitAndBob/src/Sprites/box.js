...
export class Box extends Phaser.GameObjects.Sprite {
    ...
    checkOn(time, delta) {
        if (this.scene && Utils.nothingUnder(this))
            this.activate();
        else if (this.lockedTo) {
            this.deActivate();
            //this.setPosition(this.body.x += this.isLocked.body.deltaX(), this.isLocked.y);
            //this.setPosition(this.x + this.lockedTo.body.deltaX(), this.lockedTo.body.top - this.displayHeight);
        }
    }
    gridX() {
        //move to tile grid
    ...
    preUpdate() {
        if (this.lockedTo) {
            if (this.lockedTo.body.prev.x !== this.lockedTo.body.position.x) alert('woohoo');
        }
        if (this.text) {
