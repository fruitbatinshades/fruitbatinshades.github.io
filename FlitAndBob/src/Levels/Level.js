...
export class Level extends Phaser.Scene {
    ...
    constructor(handle) {
        ...
        this.movables = new Phaser.GameObjects.Group(this,[]);
    }
    create() {
    ...
    preRender(renderer, time, delta) {
        ...
        if (this.bob.locked || this.bob.wasLocked) {
            let dx = this.bob.lockedTo.lastX - this.bob.lockedTo.body.x;
            
                this.bob.x -= dx;
                this.bob.y = this.bob.lockedTo.body.top - this.bob.body.halfHeight;
            
            if (this.bob.body.velocity.x !== 0) {
                this.bob.body.velocity.y = 0;
            }
        }
        this.movables.children.each((m) => {
            if (m.lastX !== m.body.x) console.log(m.name, m.lastX - m.body.x)
            //if (m.lastY !== m.body.y) console.log(m.name, m.lastY, m.body.y)
        });
    }
    /**
    ...
    buildLevel() {
        ...
        this.levelEvents.on('boxTileCollide', (box, tile) => {
            if (tile.constructor.name === 'InteractionZone' || tile.constructor.name === 'InteractionContainer') {
    ...
    update(time, delta) {
        ...
        this.levelEvents.emit('sceneUpdate', time, delta);

        //platform test
        //this.physics.collide(this.bob, this.movables, this.customSep, null, this);
